...
app.post('/api/trades', verifyToken, async (req, res) => {
  try {
    const { offerId: rawOfferId, listingId: rawListingId, amountBtc, amount, paymentMethod, trade_type } = req.body;
    ...
    const { data: trade, error } = await supabaseAdmin.from('trades').insert([{
      listing_id: listingId, buyer_id: req.userId, seller_id: listing.seller_id, trade_type: trade_type || 'BUY',
      amount_usd: listing.amount_usd, amount_btc: parsedAmountBtc, status: 'CREATED',
      escrow_wallet_address: escrowWallet, platform_fee_btc: parseFloat(fee),
      platform_fee_usd: (listing.amount_usd * 0.005).toFixed(2), fee_status: 'PENDING',
    ...
    await createNotification(listing.seller_id, 'trade', '💰 New Trade Request',
      `Someone wants to trade ${listing.gift_card_brand || 'Bitcoin'} worth $${listing.amount_usd}`, `/trade/${trade[0].id}`);
...
app.post('/api/trades/:id/mark-paid', verifyToken, async (req, res) => {
  try {
    ...
    if (error) return res.status(400).json({ error: error.message });
    await createNotification(trade.seller_id, 'trade', '✅ Payment Confirmed',
      `Buyer confirmed payment for trade #${req.params.id.slice(0, 8)}. Please verify and release Bitcoin.`, `/trade/${req.params.id}`);
...
app.post('/api/trades/:id/release', verifyToken, async (req, res) => {
  try {
    ...
    const allowedStatuses = ['PAYMENT_SENT', 'PAID'];
    if (!allowedStatuses.includes(trade.status)) return res.status(400).json({ error: `Cannot release — trade status is ${trade.status}. Buyer must confirm payment first.` });
    await releaseFundsToBuyer(trade.id, trade.buyer_id, trade.amount_btc, trade.amount_usd);
    await supabaseAdmin.from('company_profits').insert([{ trade_id: req.params.id, profit_btc: trade.platform_fee_btc, profit_usd: trade.platform_fee_usd, status: 'COLLECTED' }]);
    await updateUserTradeStats(trade.seller_id);
    await updateUserTradeStats(trade.buyer_id);
    await createNotification(trade.buyer_id, 'trade', '🎉 Bitcoin Released', `Trade #${req.params.id.slice(0, 8)} completed! Bitcoin sent to your wallet.`, `/trade/${req.params.id}`);
...
app.post('/api/trades/:id/cancel', verifyToken, async (req, res) => {
  try {
    ...
    const { data: trade, error: fetchError } = await supabaseAdmin.from('trades').select('*').eq('id', req.params.id).single();
    if (fetchError || !trade) return res.status(404).json({ error: 'Trade not found' });
    if (trade.buyer_id !== req.userId) return res.status(403).json({ error: 'Only the buyer can cancel this trade' });
    ...
    if (escrow && escrow.status === 'LOCKED') {
      ...
      await supabaseAdmin.from('user_balances').update({ balance_btc: newSellerBalance, updated_at: new Date() }).eq('user_id', trade.seller_id);
...
app.get('/api/notifications', verifyToken, async (req, res) => {
  try {
    const { data, error } = await supabaseAdmin.from('notifications').select('*').eq('user_id', req.userId).order('created_at', { ascending: false }).limit(50);
...
app.listen(PORT, () => {
  ...
  console.log('📋 Routes: /api/auth, /api/users, /api/listings, /api/trades, /api/my-trades, /api/wallet, /api/notifications');
});
